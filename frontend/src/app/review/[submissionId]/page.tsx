...
export default function ReviewDetailPage({ params }: { params: Promise<{ submissionId: string }> }) {
  ...
  const s: Record<string, any> = {
    ...
    textBox: {
      ...
      padding: '0.875rem', fontSize: '0.875rem', color: '#1e293b',
      lineHeight: 1.6, whiteSpace: 'pre-wrap' as const, minHeight: '60px',
    },
    textarea: {
      width: '100%', padding: '0.625rem', border: '1px solid #ddd',
      borderRadius: '6px', fontSize: '0.875rem', minHeight: '80px',
      resize: 'vertical' as const, boxSizing: 'border-box' as const,
    },
    btnGreen: {
      padding: '0.625rem 1.5rem', background: '#22c55e', color: 'white',
      border: 'none', borderRadius: '6px', cursor: 'pointer',
      fontSize: '0.875rem', fontWeight: 600, opacity: deciding ? 0.6 : 1,
    },
    btnRed: {
      padding: '0.625rem 1.5rem', background: '#ef4444', color: 'white',
      border: 'none', borderRadius: '6px', cursor: 'pointer',
      fontSize: '0.875rem', fontWeight: 600, opacity: deciding ? 0.6 : 1,
    },
    ...
    <div style={s.page}>
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#64748b', margin: 0, fontSize: '0.875rem' }}>
          ...
          {submission.submitted_at && ` • Отправлен: ${new Date(submission.submitted_at).toLocaleDateString('ru-RU')}`}
          {' • '}
          <strong style={{ color: submission.status === 'approved' ? '#22c55e' : submission.status === 'rejected' ? '#ef4444' : '#f59e0b' }}>
          ...
          {submission.kpi_values.map((kpi, idx) => (
            <div
              ...
              style={{
                ...
                paddingBottom: '1rem',
                marginBottom: idx < submission.kpi_values!.length - 1 ? '1rem' : 0,
              }}
            >
              ...
              <div style={{ fontSize: '0.8rem', color: '#64748b', marginBottom: '0.5rem' }}>
                {kpi.criterion} • Вес: {kpi.weight}% • План: {kpi.plan_value}
              </div>
              <div style={{ display: 'flex', gap: '2rem' }}>
                ...
                {kpi.base_value != null && (
                  <div>
                    <span style={{ fontSize: '0.8rem', color: '#64748b' }}>База: </span>
                    <strong>{kpi.base_value}</strong>
                  </div>
                )}
              </div>
            </div>
      ...
      {canDecide ? (
        <div style={s.card}>
          ...
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button style={s.btnGreen} onClick={() => handleDecide(true)} disabled={deciding}>
              Утвердить
            </button>
            <button style={s.btnRed} onClick={() => handleDecide(false)} disabled={deciding}>
              Вернуть на доработку
            </button>
          </div>
        </div>
      ) : (
        <div style={{ padding: '1rem', background: '#f1f5f9', borderRadius: '8px', fontSize: '0.875rem', color: '#64748b' }}>
          Статус отчёта: <strong>{STATUS_LABELS[submission.status] || submission.status}</strong> — решение уже принято.
        </div>
      )}
    </div>
  )
