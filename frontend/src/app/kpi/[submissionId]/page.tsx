...
export default function KpiFormPage({ params }: { params: { submissionId: string } }) {
  ...
  async function refreshSubmission() {
    try {
      const res = await api.get(`/submissions/my/${submissionId}`)
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // KpiEngineResult → extract kpi_results into submission.kpi_values
      const engineResult = res.data
      setSubmission(prev => prev
        ? { ...prev, kpi_values: engineResult.kpi_results, ai_generated_at: new Date().toISOString() }
        : prev
      )
      setScoreData({
        partial_score: engineResult.partial_score,
        total_weight: engineResult.total_weight,
        scored_weight: engineResult.scored_weight,
        completion_pct: engineResult.completion_pct,
      })
    } catch (err: any) {
      alert(err.response?.data?.detail || 'Ошибка генерации AI-оценки')
    } finally {
      setGenerating(false)
    }
  }
  ...
  const handleNumericUpdate = useCallback(async (criterion: string, factValue: number) => {
    try {
      ...
      // Refresh both submission and score
    ...
    <>
      ...
      {generating && (
        <div className="loader-overlay">
          ...
          <p className="loader-text">АНАЛИЗ ТРУДОЗАТРАТ...</p>
        </div>
      )}
